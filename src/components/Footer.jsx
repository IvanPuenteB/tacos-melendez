function Footer() {
  return (
    <footer className="bg-surface-container dark:bg-surface-container-low border-t border-on-surface/5 dark:border-on-surface-variant/10 w-full py-12 px-8 md:px-margin-desktop">
      <div className="flex flex-col md:flex-row justify-between items-center gap-unit max-w-container-max mx-auto text-center md:text-left">
        <div className="flex items-center gap-4 mb-6 md:mb-0">
          <img
            alt="Tacos Meléndez Logo"
            className="w-8 h-8 rounded-full grayscale opacity-80"
            src="https://lh3.googleusercontent.com/aida-public/AB6AXuACXGMbXg6a3CKpe2jQNd4tIM-zmA2MytQBwIXNcDWCc8W5WLGcFUVf4YRag-em2sdcBJF_TBSJR0YRLs3Cv-MuqzDzM9iJbyd9PD_K4Crm33oKdJQxy4_QjEpYRsvQp_h1n9Qrvch_q9zksNO3TifxRuHFyeQgPgHNqwEqS3jf65n_yzG4XWR_WNbQqFnV5LGaLp0SNDt1CW2FORymuaoUl9-V_C2m_Ku24ZT7UDkGnp5n5LD4q1yR3-kLBALDuMw83Q"
          />
          <span className="font-headline-md text-headline-md text-primary dark:text-primary-fixed-dim">
            Tacos Meléndez
          </span>
        </div>
        <nav className="flex gap-6 mb-6 md:mb-0">
          <a
            className="font-label-sm text-label-sm text-on-surface-variant dark:text-outline-variant hover:text-primary dark:hover:text-primary-fixed-dim transition-colors inline-flex items-center gap-1"
            href="#menu"
          >
            <span class="">Volver al menú</span>
          </a>
        </nav>
        <div class="flex items-center gap-3 mb-6 md:mb-0">
          <a
            href="https://facebook.com"
            target="_blank"
            rel="noopener noreferrer"
            aria-label="Facebook de Tacos Meléndez"
            class="flex items-center gap-2 px-3 py-1.5 border border-[#2C1E1E]/20 bg-surface text-on-surface-variant hover:text-primary hover:border-primary transition-colors text-label-sm font-label-sm rounded-sm"
          >
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"></path>
            </svg>
            <span class="">Facebook</span>
          </a>
          <a
            href="https://tiktok.com"
            target="_blank"
            rel="noopener noreferrer"
            aria-label="TikTok de Tacos Meléndez"
            class="flex items-center gap-2 px-3 py-1.5 border border-[#2C1E1E]/20 bg-surface text-on-surface-variant hover:text-primary hover:border-primary transition-colors text-label-sm font-label-sm rounded-sm"
          >
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.24 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"></path>
            </svg>
            <span class="">TikTok</span>
          </a>
        </div>
        <div>
          <p className="font-body-md text-body-md text-secondary dark:text-secondary-fixed-dim">
            © 2026 Tacos Meléndez. Hechos con amor para ti.
          </p>
        </div>
      </div>
    </footer>
  );
}

export default Footer;
